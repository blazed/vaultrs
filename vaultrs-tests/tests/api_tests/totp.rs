...
mod key {
    ...
    pub async fn test_create(endpoint: &TotpEndpoint<'_>) {
        ...
        let resp = key::create(endpoint.client, &endpoint.path, &endpoint.name, Some).await;
        assert!(resp.is_ok());
    }
...
mod code {
    pub async fn test_generate(endpoint: &TotpEndpoint<'_>) {
        let resp = code::generate(endpoint.client, &endpoint.path, &endpoint.name).await;
        assert!(resp.is_ok());
    }

    pub async fn test_validate(endpoint: &TotpEndpoint<'_>) {
        let resp = code::validate(endpoint.client, &endpoint.path, &endpoint.name, "123456").await;
        assert!(resp.is_ok());
    }
}
